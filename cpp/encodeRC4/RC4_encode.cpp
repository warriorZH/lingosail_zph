...
void RC4::file_encode(string const input_path, string const output_path)
{
    ...
    ifstream input_fd(input_path.c_str(), ios::in);
    ofstream output_fd(output_path.c_str(), ios::out|ios::binary);
    if(!input_fd.is_open())
    {
        cout<<"encode open input file error!!"<<endl;
        //exit(1);
    }
    if(!output_fd.is_open())
    {
        cout<<"encode open output file error!!"<<endl;
        //exit(1);
    }
...
void RC4::file_decode(string const input_path, string const output_path)
{
    ...
    ofstream output_fd(output_path.c_str(), ios::out);
    if(!input_fd.is_open())
    {
        cout<<"decode open input file error!!"<<endl;
        //exit(1);
    }
    if(!output_fd.is_open())
    {
        cout<<"decode open output file error!!"<<endl;
        //exit(1);
    }
    istreambuf_iterator<char> beg(input_fd), end;
    string strdata(beg, end);
    ...
    //cout<<"input_line:"<<inputline.c_str()<<endl;
    this->PRGA_function(inputline.length());
    this->RC4_decoder(inputline, outputline);
    string outputTMP="";
    __SIZE_TYPE__ outputIndexF=0;
    __SIZE_TYPE__ outputIndexB=0;
    __SIZE_TYPE__ tmp = 0;
    while(outputIndexB < (outputline.length()-1))
    {
        outputIndexB = outputline.find_first_of("\n", tmp);
        //outputIndexB += 1;
        outputTMP = "";
        outputTMP = outputline.substr(outputIndexF, outputIndexB-outputIndexF+1);
        //cout<<"output_line:"<<outputTMP.c_str()<<endl;
        //cout<<"outputIndexB:"<<outputIndexB<<endl;
        //cout<<"outputIndexF:"<<outputIndexF<<endl;
        outputIndexF = outputIndexB + 1;
        tmp = outputIndexB + 1;
        outputTMP.c_str();
        output_fd.write(outputTMP.c_str(), outputTMP.length());
    }
